...
test('ctor requires callbacks in each form', function() {
    ...
    var lc = new LawnchairNcc({name:store.name}, function(err, ref) {
...
test('independent data stores', function() {
    ...
    new LawnchairNcc({name: "store1"}, function(err, store1) {
        store1.nuke(function(){
            store1.save({key: 'apple', quantity: 3}, function() {
                new LawnchairNcc({name: "store2"}, function(err, store2) {
                    store1.all(function(err, r) {
                        equals(r.length, 1);
                        store2.all(function(err, r) {
                            equals(r.length, 0);
                            QUnit.start();
                        });
                    });
...
test('chainable', function() {
    ...
    same(store.all(function(err, r) { QUnit.start(); }), store, 'should be chainable (return itself)');
})
...
test('scoped variable in shorthand callback', function() {
    ...
    var tmp = new LawnchairNcc({name:'temps', record:'tmp'}, function() {
		this.nuke(function() {
...
test( 'saving objects', function() {
    ...
    store.save(me, function() {
        store.save({key:"something", value:"else"}, function(err, r) {
            store.all(function(err, r) {
    ...
    store.save({key:"repeated",field:"first"}, function() {
        store.save({key:"repeated", field:"second"}, function() {
            store.all(function(err, r) {
                ...
                store.get("repeated", function(err, obj) {
...
test( 'save without callback', function() {
    ...
    store.save(me, function(err, obj) {
        var key = obj.key;
        store.save(obj);
...
test('batch update twice', function(){
    ...
    store.batch([{key:'twice',val:"original"}], function() {
        store.batch([{key:"once", val: 1},{key:'twice',val:"stale"}], function(err, rs) {
            store.get("once", function(err, obj){
                equals(obj.val, 1, "update once should work next to update twice");
                store.get("twice", function(err, obj){
...
test('get functionality', function() {
    ...
    store.save({key:'xyz', name:'tim'}, function() {
        store.get('xyz', function(err, r) {
            ...
            store.get('doesntexist', function(err, s) {
...
test('get batch functionality', function() {
    ...
    store.batch(t, function() {
        this.get(['test-get','test-get-1'], function(err, r) {
...
test( 'full callback syntax', function() {
    ...
    store.get('somekey', function(err, r){
...
test('exists functionality', function(){
    ...
    QUnit.stop();
    store.save({key:'xyz', name:'tim'}, function() {
        store.exists('xyz', function(err, r) {
            equals(r, true, 'should exist after save');
            store.exists('imaginary', function(err, r) {
...
test( 'full callback syntax', function() {
    ...
    store.save({key:'somekey', name:'something'}, function() {
        store.remove('somekey', function(err, r){
            ok(true, 'callback got called');
            same(this, store, '"this" should be the Lawnchair instance');
            QUnit.start();
        });
...
test( 'remove functionality', function() {
    ...
    store.save({name:'joni'}, function(err, r) {
        //store.find("r.name == 'joni'", function(r){
            store.remove(r, function(err, r) {
                store.all(function(err, all) {
                    ...
                    store.save({key:'die', name:'dudeman'}, function(err, r) {
                        store.remove('die', function(err, r){
                            store.all(function(err, rec) {
...
test( 'remove functionality (part 2)', function() {
    ...
    store.save({name:'joni'}, function(err, r) {
        store.save({name:'mitchell'}, function(err, r2) {
            store.remove(r, function(err, r) {
                        ...
                        store.remove('die', function(err, r){
                            store.all(function(err, rec) {
                                ...
                                store.keys(function(err, keys) {
                                    ...
                                    store.remove('xyz', function(err, r) {
                                        store.keys(function(err, keys) {
...
test( 'batch add, get, and remove', function() {
    ...
    store.batch([{name:'joni', order: 0},
                 ...
                 {name: 'the great quux', order: 4}], function(err, results) {
        ...
        store.remove(['foo', results[3] /*bat, by object*/], function() {
            store.get([results[0].key, 'lastname', 'foo', 'bat',
                       results[4].key], function(err, results) {
